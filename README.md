...
> **AI Disclaimer:** This fork was created with the assistance of AI (opencode). The jailbreak-to-jailed conversion, code changes, Xcode project setup, and documentation were all generated by AI. Use at your own risk — this has not been audited by the original Juice developers or any security researcher.

Jail-Juice is a **jailed** (non-jailbroken) fork of [Juice](https://github.com/ExoCore-Kernel/Juice), an experimental Wine GUI runtime for Windows applications on iPhone and iPad. The bundled Wine runtime is named **Grape**. This repository contains the UIKit app, launch helpers, complete modified Wine 11.13 source, the same Wine delta as an auditable patch, packaging assets, FEX integration, and the build system used to produce Jail-Juice.
...
Jail-Juice runs Windows programs without the normal iOS app sandbox and exposes the host filesystem through Wine's Z: drive. Only run software you trust. Read [Security](SECURITY.md) before testing or distributing it.

Wine remains LGPL-2.1-or-later under its in-tree license files. Juice-original code is licensed under the MIT license.

## Disclaimer

**This is an AI-generated fork.** The jailbreak-to-jailed conversion was performed entirely by AI (opencode). The original [Juice](https://github.com/ExoCore-Kernel/Juice) project by ExoCore-Kernel is not affiliated with or responsible for this fork. The AI-generated changes have not been audited for security, correctness, or stability. Use this software at your own risk.
